...
- **Related ledger records:** `DECISIONS-CliInvoke-v3-internals-visibility.md` — D002, D003, D004, D005, D006
...
In this pass, remove the unused IVT grants now; schedule the reduction of *required* grants as follow-up work sequenced behind the D002 principle.

- **Driver:** the user wants fewer coupling points without premature breaking public-API changes.
- **Resolution:** Option B — Unused now, required later: remove unused grants now; schedule required-grant reduction after D002 is set.
- **Normalized requirement:** Remove unused IVT grants in this pass. Reduction of the required grants shall be scheduled as follow-up work sequenced behind the D002 principle.
...
For each required IVT grant, the reduction mechanism (promote the specific needed type to public API, relocate it to CliInvoke.Core, or another approach) shall be decided **per internal type**, applying the D002 principle case-by-case.
...
- **Normalized requirement:** For each required IVT grant, the reduction mechanism shall be decided per internal type during the follow-up work, applying D002 case-by-case. No single global mechanism is mandated.
...
- **Resolution:** Deferred in the original decision pending the D005 per-type analysis; once known, apply per-type — public promotion = v3 major, Core relocation = non-breaking.
...
### D007 — Documentation without a CI guard
