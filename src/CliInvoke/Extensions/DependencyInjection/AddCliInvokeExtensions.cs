...
/// <summary>
///     Provides extension methods for setting up dependency injection for CliInvoke's core
///     services and built-in middleware.
/// </summary>
/// <remarks>
///     <para>
...
///         (<c>PowerShellMiddleware</c>, <c>CmdMiddleware</c>) is not registered here; call
///         <c>AddCliInvokeSpecializations</c> from that package alongside <c>AddCliInvoke</c>
///         when using <c>UsePowerShell()</c> or <c>UseCmd()</c>.
///     </para>
/// </remarks>
...
/// </summary>
    ...
    extension(IServiceCollection services)
    {
        ...
        /// </exception>
        public IServiceCollection AddCliInvoke(Action<IProcessMiddlewareBuilder> configure,
            ServiceLifetime lifetime = ServiceLifetime.Scoped)
        {
    ...
    extension(IServiceCollection services)
    {
        ...
        /// </summary>
        private IServiceCollection ConfigureCliInvokeServices(Action<IProcessMiddlewareBuilder>? configure,
    ...
    {
        /// <summary>
        ///     Registers CliInvoke's core built-in middleware types in the service collection so the
        ///     type-based <see cref="IProcessMiddlewareBuilder.UseMiddleware{T}"/> overload
        ///     (used by the convenience extensions <c>UseLogging</c> and the retry extensions)
        ...
        ///     when no <see cref="ILogger{T}"/> is registered. Platform middleware
        ///     (<c>PowerShellMiddleware</c>, <c>CmdMiddleware</c>) is not registered here; users of the
        ///     CliInvoke.Specializations package register it via that package's
        ///     <c>AddCliInvokeSpecializations</c>.
        /// </remarks>
        /// <param name="lifetime">The service lifetime to register the middleware with.</param>
        private void RegisterBuiltInMiddleware(ServiceLifetime lifetime)
        {
