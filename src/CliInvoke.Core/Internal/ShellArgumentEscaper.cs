...
/// <remarks>
///     <para>
///         The shell wrappers in <c>CliInvoke.Specializations</c> (PowerShell / cmd
///         middleware) and the generic <c>RunnerConfigurationFactory</c> rebuild the
///         caller's <c>TargetFilePath</c> + <c>Arguments</c> into a single shell
///         command line (e.g. <c>cmd /c "target" args</c> or
///         <c>pwsh -Command &amp; "target" args</c>). The shell then re-parses that
///         line, so any shell metacharacters in the original arguments would
///         otherwise execute as additional shell commands (command injection). These
///         helpers neutralise the dangerous metacharacters for each shell before the
///         command is rebuilt.
///     </para>
///     <para>
///         Lives in <c>CliInvoke.Core</c> so both <c>CliInvoke</c> (the runner
///         factory) and <c>CliInvoke.Specializations</c> (the middleware) can use it
///         without a circular dependency between the two.
///     </para>
/// </remarks>
public static class ShellArgumentEscaper
{
    /// <summary>
