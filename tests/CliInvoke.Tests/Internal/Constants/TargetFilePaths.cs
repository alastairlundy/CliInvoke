...
{
    private static readonly IExecutableFileResolver _filePathResolver = new ExecutableFileResolver(new ExecutableFileDetector(),
    ...
    public static string DotnetFilePath
    {
        get
        {
            Task<FileInfo> resultTask = _filePathResolver.LocateExecutableAsync(OperatingSystem.IsWindows() ? "dotnet.exe" : "dotnet", SearchOption.AllDirectories, CancellationToken.None);
