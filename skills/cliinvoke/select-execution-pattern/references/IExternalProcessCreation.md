...
### Note on Middleware
Middleware is an **invoker-level** concern: it wraps `IProcessInvoker`/`ProcessInvoker`, not the `IExternalProcess` factory. The factory path above is unaffected by middleware — there is no middleware API at the `IExternalProcess` layer. If you need cross-cutting concerns (logging, post-exit validation, platform wrapping), configure them on a `ProcessInvoker` via the `Use*` extension methods and let the invoker drive the factory.
