...
- `AddCliInvokeSpecializations` (CliInvoke.Specializations package) registers only
  `PowerShellMiddleware`, `CmdMiddleware`, `DefaultShellMiddleware`, and `ShellMiddlewareOptions`.
...
- `AddCliInvoke(builder => builder.UsePowerShell())` without `AddCliInvokeSpecializations()`
  ...
  when the invoker is first resolved. The same applies to `UseCmd()` and `UseDefaultShell()`.
